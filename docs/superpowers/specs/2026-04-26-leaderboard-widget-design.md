...
| `target_hit_celebration` | Personal variant + `current_value ≥ target_value`, fires once on mount | Layered on top of `mounting`: Trophy icon retones to `--color-text-success` at `~520ms` (after ring completes), one-cycle 280ms springy pulse on the ring container's `::before` halo at `~900ms` (Earnings sheen vocabulary, the frame the ring stroke completes — `COUNT_UP_DELAY_MS=180 + COUNT_UP_MS=720`), brightness settle on the ring frame at `~900ms` (360ms `lbRingFrameSettle`, Earnings `earnTotalSettle` vocab), count-up tint wash at `~940ms` (transient — resolves back to `--grey-90`, never permanently retones the digit). Verdict word swaps to "Target hit" and pops on the springy curve at `~1080ms`. **No confetti, no illustrations, no "Great job!" copy** (§18 #11 — the chip + tone retone + ring sweep carry the celebration). |
